...
| 9     | Ask to buy, order conversation    | done        | feat/complete-shop                      | Verified 2026-09-04 over real HTTP as a real buyer: create_order via the RPC with payment_method link priced 2 x 1800 = 3600, order CY-2609-0001. Ask to buy form prefills from the profile, calls the RPC, opens an order conversation seeded with what they want, empties the cart, lands on the order page. |
| 9b    | Payment links and marking paid    | done        | feat/complete-shop                      | Payment link box on /admin/orders/[id] posts a payment_link card into the order's conversation. Mark paid and mark refunded are owner only Server Actions and write order_events. Verified 2026-09-04 that a buyer setting payment_status=paid on their own order is refused by trigger, and the owner is not. |
| 10    | Orders, email, fulfilment         | done        | feat/complete-shop                      | /order/[orderNumber]?t= opens with no session via get_order_by_token; wrong token returns null, verified. /admin/orders list with tabs and search, detail with stepper, tracking, label block, notes, history. Four Resend emails rendered to text and HTML, skipped and logged when no key. NOT verified: an email actually arriving, since no RESEND_API_KEY is set. |
| 11    | Chat                              | done        | feat/complete-shop                      | Buyer /messages list and /messages/[id] thread, owner inbox and thread, Realtime on inserts filtered by conversation_id, pictures to the private bucket under the thread prefix, delivery details Dialog that re-quotes server side, floating launcher. Verified live: buyer text 201, buyer faking payment_link refused, all card types render. NOT verified: a Realtime message arriving without refresh, which needs two real browsers. |
| 12    | Custom requests                   | done        | feat/complete-shop                      | /custom with up to five reference pictures, a custom_request_details row, a prose first message, confirmation page, the withdrawal note said once. Admin sees a magenta tag, the details panel with the pictures, and a send a quote action that makes a one off product tagged quote and hidden from every listing. |
| 13    | Polish                            | done        | feat/complete-shop                      | Legal pages with marked TODO blocks, sitemap and robots, JSON-LD on product pages, 404 and error boundary, skeleton loading states, copy audit clean (no dashes, no banned words), two stale .gitkeep removed. NOT done: Lighthouse numbers, since headless Chrome here has no compositor and its scores are not real. Run from DevTools on the live domain. |
| 14    | Deploy preparation                | done        | feat/complete-shop                      | src/lib/env.ts parses at boot and fails with a sentence. CI runs typecheck, lint, tests, build and greps client chunks for the service role key and the admin client (clean locally). HSTS, nosniff, DENY, referrer and permissions policies static; CSP with a per request nonce in middleware, verified 40 of 40 script tags nonced. DEPLOY.md written. |
...
- 2026-09-04 Both Supabase projects pause on the free tier after a week idle, and paused
  twice during this build. The site is down while paused. Pro on production is the fix.
  See MANUAL_STEPS.md section 1.
- 2026-09-04 RESOLVED the same day: a row policy added for buyer delivery updates combined
  with 0001's table wide UPDATE grant on orders let a buyer set their own payment_status.
  Closed by orders_guard_buyer_update before the branch merged. Proven refused live.
- 2026-09-04 Production has zero migrations, no owner, no domain, no Resend. DEPLOY.md is
  the ordered list. Nothing in code is blocked on any of it.

- 2026-08-07 Stripe test keys are no longer a blocker for anything: card
  payment moved to the backlog. Stage 9 needs no external credential at all.
- 2026-08-20 RESOLVED, was: SECURITY, any signed in buyer could make
  ...
  Closed 2026-08-20 by migration 20260820151922_lock_down_profile_roles:
  column level grants (display_name, email, phone only, so `role` is not
  updatable at all) plus a `set_user_role` SECURITY DEFINER function that is
  owner only, writes an audit row to `role_changes`, and refuses to demote the
  last owner. Proven exploitable before and refused after.
...
- 2026-08-07 Build order changed to 11, then 10, then 9, then 9b. Moving payment into the conversation made stages 9 and 9b depend on chat and on order pages, both of which come later in the plan's numbering. Building in dependency order means nothing gets half-built and returned to; the numbers stay as they are, only the order of work changes.
- 2026-08-07 PLAN CHANGE: payment happens in the product conversation, not at a checkout. The seller sends a link, most likely Revolut, and marks the order paid when the money arrives. Stripe, cash on delivery and bank transfer moved to the backlog in BUILD_PLAN.md "After launch", behind the `PaymentMethod` interface, the existing `payment_method` enum values and the Stripe columns already on orders, so adding one later is an implementation rather than a rewrite. Old prompt 9 is recoverable from git history.
- 2026-08-07 PLAN CHANGE: delivery details are collected in a Dialog opened from inside the conversation, not on a checkout page. Half of these buyers collect at a market and never need an address, so asking every one of them for a postcode up front was asking the wrong question first. Re-quoting still happens server side from the database when an address is entered.
- 2026-08-07 A `link` value is being added to the `payment_method` enum rather than reusing `bank_transfer` for a Revolut link. Reusing it would make the orders list lie about how people actually paid, and that list is what the owner reconciles against.
- 2026-08-07 The `create_order` RPC survives the payment plan change untouched. It never knew about Stripe: it validates, prices and writes the order, and which payment method is recorded is a parameter. That is the adapter boundary in CLAUDE.md section 7 doing its job.
- 2026-08-07 PLAN CHANGE, at the owner's request: navigation is a left-anchored drawer behind a menu button, not the "horizontally scrollable row" of category chips BUILD_PLAN.md prompt 6 specified. Swiping a half-hidden strip sideways is a poor way to learn what a shop sells, and the drawer also absorbed the account links that were overflowing the header. Chips survive on `/shop` as filters rather than navigation, and wrap instead of scrolling.
- 2026-08-07 Cyprus is deliberately excluded from the EU country set in `resolveZone`, despite being an EU member. A parcel inside Cyprus is a different proposition from one to Ireland, and the seeded rates price them separately, so it gets its own zone.
- 2026-08-07 The BOX NOW provider still quotes with no credentials, because rates come from our own `shipping_methods` table rather than their API. Only booking, labels and tracking need the account. That means a locker price shows on checkout long before BOX NOW is approved, which is the behaviour SETUP.md section 5 wants given account approval is the long pole.
- 2026-08-07 `LockerPicker` falls back to a plain locker id field with a link to the BOX NOW finder when their CDN script fails or the partner id is missing. Someone else's script going down must never block checkout.
- 2026-08-07 Vitest was added this stage, per PLAN.md's global exit checks. `npm test` runs it. The config is `vitest.config.mts` rather than `.ts` because Vite warns about ESM syntax in a file it loads as CommonJS otherwise.
- 2026-08-07 There are two rate test suites on purpose: one on synthetic fixtures for the edge cases (exact threshold, 1g over a tier, null weight limit), and one pinned to the seven rows actually seeded from SETUP.md section 5, copied out of the dev database. The second catches a rate table change that the first would sail past.
